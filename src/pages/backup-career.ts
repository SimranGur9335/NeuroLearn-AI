...
const CareerGuidance = () => {
    ...
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                const res = await apiFetch('/v1/career/profile');
                if (res.ok) {
                    const data = await res.json();
                    if (data) {
                        setResumeText(data.resume_text || '');
                        setSelectedPathId(data.target_career || 'ai-engineer');
                        setCustomSkills(data.custom_skills || []);
                        setAiAnalysis(data.ai_analysis || null);
                        localStorage.setItem('neurolearn_target_career', data.target_career || 'ai-engineer');
                    }
                }
            } catch (err) {
    ...
    // Set the selected path, sync with local storage, and save to backend
    const handleSelectPath = async (id) => {
        setSelectedPathId(id);
        localStorage.setItem('neurolearn_target_career', id);
        try {
            await apiFetch('/v1/career/profile', {
                method: 'POST',
    ...
    // Remove custom skills tag
    const handleRemoveSkill = (tag) => {
        setCustomSkills(prev => prev.filter(s => s !== tag));
    };
    ...
    const handleSaveProfile = async () => {
        try {
            ...
            if (res.ok) {
                ...
                alert("Failed to save career settings.");
            }
        } catch (err) {
            console.error(err);
    ...
    // Trigger AI analysis diagnostics
    const handleTriggerAnalysis = async () => {
        ...
        try {
            ...
            const res = await apiFetch('/v1/career/analyze', { method: 'POST' });
            if (res.ok) {
                const data = await res.json();
                setAiAnalysis(data);
                alert("AI diagnostics review calculated successfully!");
            } else {
                alert("Error executing AI Analysis. Fallback metrics loaded.");
            }
        } catch (err) {
            console.error(err);
            alert("Network error triggering AI Diagnostics.");
        } finally {
    ...
    const handleEvaluateAnswer = async (qText, qTopic) => {
        if (!userAnswer.trim()) {
        ...
        try {
            const res = await apiFetch('/v1/career/interview/evaluate', {
                method: 'POST',
                body: JSON.stringify({
                    question: qText,
                    answer: userAnswer,
            ...
            console.error(err);
            alert("Network error during grading evaluation.");
        } finally {
    ...
    // Compute Skill Gaps matching (Either from AI analysis if available, or direct calculations)
    const skillAnalysis = useMemo(() => {
        if (aiAnalysis && aiAnalysis.skill_gap_analysis) {
            return {
                completed: aiAnalysis.skill_gap_analysis.acquired || [],
                inProgress: inProgressSkills.filter(s => allRequiredSkills.includes(s)),
                missing: aiAnalysis.skill_gap_analysis.missing || []
            };
        }
    ...
    }, [allRequiredSkills, acquiredSkills, inProgressSkills, aiAnalysis]);

    // Compute readiness
    const readinessMetrics = useMemo(() => {
        const score = aiAnalysis && aiAnalysis.readiness_score !== undefined
            ? aiAnalysis.readiness_score
            : (() => {
                ...
                return Math.min(100, Math.round(baseMatchPercent * 0.9 + xpBonus));
            })();
    ...
    }, [allRequiredSkills, skillAnalysis, xp, aiAnalysis]);

    // Determine next milestone
    const nextMilestone = useMemo(() => {
        const getMilestoneForSkill = (skill) => {
            const sk = skill.toLowerCase();
            if (sk.includes('python') || sk.includes('algebra')) return { domainId: 'ai-ml', nodeName: 'Python Foundations & Linear Algebra' };
            if (sk.includes('supervised') || sk.includes('unsupervised') || sk.includes('scikit')) return { domainId: 'ai-ml', nodeName: 'Supervised & Unsupervised Learning' };
            if (sk.includes('deep learning') || sk.includes('neural') || sk.includes('pytorch') || sk.includes('tensorflow')) return { domainId: 'ai-ml', nodeName: 'Deep Learning & Neural Networks' };
            if (sk.includes('generative') || sk.includes('llm') || sk.includes('transformer')) return { domainId: 'ai-ml', nodeName: 'Generative AI & LLMs' };
            return null;
        };
    ...
    const filteredResources = useMemo(() => {
        ...
        if (resourceCategory === 'All') {
            baseList = Object.entries(RESOURCE_LIBRARY).flatMap(([cat, list]) =>
                list.map(r => ({ ...r, category: cat }))
            );
        } else {
            baseList = RESOURCE_LIBRARY[resourceCategory]?.map(r => ({ ...r, category: resourceCategory })) || [];
        }

        if (aiAnalysis && aiAnalysis.learning_suggestions) {
            const aiSugs = aiAnalysis.learning_suggestions.map((s, i) => ({
                title: s.title,
                category: s.type === 'quiz' ? 'Quizzes (AI Recommended)' : 'Resources (AI Recommended)',
                provider: s.type === 'certification' ? 'AWS / Academy' : 'NeuroLearn Platform',
                url: s.url,
                desc: `AI-Recommended study path targeting your specific gaps in ${activeCareer.title}.`
            }));
            return [...aiSugs, ...baseList];
        }

        return baseList;
    }, [resourceCategory, aiAnalysis, activeCareer]);
...
className = "space-y-6"
    ...
    < div className = "flex flex-col md:flex-row justify-between items-start md:items-center gap-4" >
        ...
        <p className="text-xs text-indigo-500 font-bold uppercase tracking-wider" > Placement & Career Intelligence </p>
            < h2 className = "text-2xl font-black text-slate-800 dark:text-white" > AI Placement Guidance & Roadmap Operating System </h2>
                < p className = "text-slate-550 dark:text-slate-400 text-xs mt-1" >
                    Optimize resumes, review skill gaps via Gemini LLM diagnostics, and prepare with live mock interview questions.
          </p>
                        </div>
...
{/* Navigation tabs */ }
<div className="flex flex-wrap gap-1.5 border-b border-slate-250 dark:border-slate-850 pb-px" >
{
    [
    { id: 'dashboard', label: 'Command Center', icon: Cpu },
    { id: 'resume', label: 'AI Resume & Gaps', icon: FileText },
    { id: 'interviews', label: 'Mock Interview Arena', icon: Sparkles },
    ...
    { id: 'trends', label: 'Industry Trends', icon: TrendingUp }
        ...
        onClick = {() => {
            ...
            setUserAnswer('');
              ...
              className = {`flex items-center gap-1.5 px-4 py-2 text-xs font-bold transition-all border-b-2 -mb-0.5 cursor-pointer ${isActive
...
className = "flex flex-col items-center justify-center py-20 space-y-4"
    ...
    <div className="w-12 h-12 border-4 border-indigo-500/20 border-t-indigo-500 rounded-full animate-spin" />
        <p className="text-sm font-bold text-slate-700 dark:text-slate-350" >
            Running Gemini AI Profile Analytics...
</p>
    < p className = "text-xs text-slate-400" >
        Evaluating resume structures, matching skill gaps, and generating mock interview questions.
              </p>
            </motion.div>
          ) : (
    <motion.div
...
<div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 flex flex-col justify-between shadow-sm" >
    ...
    <span className="text-xs font-bold uppercase tracking-wider text-slate-400" > Target Role Profile </span>
        < h3 className = "text-lg font-extrabold text-slate-800 dark:text-white mt-2 leading-tight" >
            ...
            < p className = "text-slate-550 dark:text-slate-400 text-xs mt-2 leading-relaxed line-clamp-3" >
...
<div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 flex flex-col justify-between shadow-sm border-l-4 border-l-indigo-500" >
    ...
    <div className="flex items-center gap-1.5 text-indigo-500" >
        <Sparkles size={ 14 } className = "shrink-0" />
            <span className="text-xs font-bold uppercase tracking-wider" > Next Suggested Goal </span>
                </div>

{
    nextMilestone.domainId ? (
        ...
        < h4 className = "font-extrabold text-slate-850 dark:text-white text-sm mt-2" >
...
className = "bg-gradient-to-br from-indigo-50 to-indigo-100/30 dark:from-slate-900 dark:to-slate-950 border border-indigo-500/10 rounded-3xl p-6 space-y-4"
    >
    <div className="flex items-center gap-2 text-indigo-500" >
        <Sparkles size={ 18 } />
            < h4 className = "text-sm font-bold uppercase tracking-wider" > AI Diagnostic Analysis: { activeCareer.title } Alignment </h4>
                </div>
                < p className = "text-xs text-slate-700 dark:text-slate-350 leading-relaxed font-medium" >
                    { aiAnalysis.readiness_summary }
                    </p>

                    < div className = "grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-indigo-500/10" >
                        <div className="space-y-2" >
                            <span className="text-[10px] text-slate-400 block font-bold uppercase" > Candidate Strengths </span>
                                < ul className = "list-disc list-inside text-xs text-slate-650 dark:text-slate-300 space-y-1" >
                                {
                                    aiAnalysis.placement_readiness?.strengths?.map((str, i) => (
                                        <li key= { i } > { str } </li>
                                    ))
                                }
                                    </ul>
                                    ...
                                    < div className = "space-y-2" >
                                        <span className="text-[10px] text-slate-400 block font-bold uppercase" > Focus Areas for Improvement </span>
                                            < ul className = "list-disc list-inside text-xs text-slate-650 dark:text-slate-300 space-y-1" >
                                            {
                                                aiAnalysis.placement_readiness?.weak_areas?.map((weak, i) => (
                                                    <li key= { i } > { weak } </li>
                                                ))
                                            }
                                            </ul>
                                                    ...
                                                    < div className = "p-4 bg-slate-500/5 border border-slate-500/10 rounded-2xl flex items-center gap-4" >
                                                        <div className="p-3 bg-slate-500/10 text-slate-450 rounded-xl" >
                                                            <X size={ 20 } />
                                                                ...
                                                                < div >
                                                                <span className="text-[10px] text-slate-400 block font-semibold uppercase" > Missing Competencies </span>
        ...
        < div className = "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" >
        {
            CAREER_ROLES.map((role) => {
                ...
                onClick = {() => handleSelectPath(role.id)
                            ...
                            className = {`bg-white dark:bg-slate-900 border rounded-3xl p-5 shadow-sm transition-all duration-200 cursor-pointer flex flex-col justify-between hover:-translate-y-0.5 group ${isCurrent
            ...
            <div className="flex justify-between items-start mb-3" >
                <span className={
                    `text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${role.difficulty === "Beginner" ? 'bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-450' :
                        role.difficulty === "Intermediate" ? 'bg-yellow-100 text-yellow-700 dark:bg-yellow-950/40 dark:text-yellow-450' :
                            'bg-purple-100 text-purple-700 dark:bg-purple-950/40 dark:text-purple-450'
                    }`
}>
    { role.difficulty }
    </span>
...
{ role.demand } Demand
    ...
    < h4 className = "font-extrabold text-slate-850 dark:text-white text-sm group-hover:text-indigo-500 transition-colors" >
        { role.title }
        </h4>
    ...
    < span className = "text-indigo-650 dark:text-indigo-400 font-extrabold text-[11px] flex items-center gap-0.5 group-hover:translate-x-0.5 transition-transform" >
        Analyze
...
{/* 2. AI Resume & Skills Settings Tab */ }
{
    activeTab === 'resume' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" >
            {/* Left columns for editing resume and skills */ }
            < div className = "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm lg:col-span-2 space-y-6" >
                <div>
                <h3 className="font-extrabold text-slate-800 dark:text-white text-sm uppercase tracking-wider" > AI Resume Builder & Diagnostics </h3>
                    < p className = "text-slate-500 text-xs mt-1" >
                        Paste your resume or details and key skills.Gemini will check formats, parse alignments, and suggest improvements.
                      </p>
                            </div>
                            ...
                            < div className = "space-y-4" >
                                {/* Resume textarea */ }
                                < div className = "space-y-2" >
                                    <label className="text-xs font-bold text-slate-500 block" > Resume text content </label>
                                        < textarea
    placeholder = "Paste your resume content, projects descriptions, education, or employment details here..."
    value = { resumeText }
    onChange = {(e) => setResumeText(e.target.value)
}
rows = { 12}
className = "w-full bg-slate-50/50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-2xl p-4 text-xs font-medium focus:outline-none focus:border-indigo-500 text-slate-750 dark:text-slate-300 placeholder-slate-400"
    />
    </div>

{/* Custom skills tagging input */ }
<div className="space-y-3" >
    <label className="text-xs font-bold text-slate-500 block" > Custom profile skills(Self - declared tags) </label>
        < div className = "flex gap-2" >
            <input
                            type="text"
placeholder = "Add skill tag (e.g. Scikit-learn, React)..."
value = { skillInput }
onChange = {(e) => setSkillInput(e.target.value)}
onKeyDown = {(e) => e.key === 'Enter' && handleAddSkill()}
className = "bg-slate-50/50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-xl px-3 py-2 text-xs text-slate-800 dark:text-white focus:outline-none focus:border-indigo-500"
    />
    <button
                            onClick={ handleAddSkill }
className = "p-2.5 bg-indigo-500 text-white rounded-xl hover:bg-indigo-650 cursor-pointer"
    >
    <Plus size={ 16 } />
        </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2 pt-1" >
            {
                customSkills.map(tag => (
                    <span 
                                key= { tag } 
                                className = "bg-indigo-500/10 border border-indigo-500/20 text-indigo-650 dark:text-indigo-400 text-xs font-bold px-3 py-1 rounded-xl flex items-center gap-1.5"
                    >
                    { tag }
                    < X 
                                  size = { 12} 
...
className = "px-5 py-2.5 border border-slate-200 dark:border-slate-800 rounded-xl text-xs font-bold hover:bg-slate-50 dark:hover:bg-slate-950 cursor-pointer transition-colors"
    >
    Save Settings
        </button>
...
{/* Right side for Suggestions */ }
<div className="space-y-6" >
    {/* Resume Suggestions Card */ }
    < div className = "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4" >
        <h4 className="text-xs font-bold uppercase tracking-wider text-slate-405 flex items-center gap-1.5" >
            <ShieldAlert size={ 14 } className = "text-indigo-500" />
                AI Resume Suggestions
                    </h4>
{
    aiAnalysis && aiAnalysis.resume_suggestions && aiAnalysis.resume_suggestions.length > 0 ? (
        <div className= "space-y-3" >
        {
            aiAnalysis.resume_suggestions.map((sug, i) => (
                <div key= { i } className = "p-3 bg-indigo-500/5 border border-indigo-500/10 rounded-2xl flex items-start gap-2.5" >
                <CheckCircle2 size={ 16} className = "text-indigo-500 shrink-0 mt-0.5" />
                <p className="text-slate-750 dark:text-slate-300 text-xs leading-relaxed font-semibold" >
                { sug }
                </p>
            </div>
            ))
        ...
        <FileText size={ 32 } className = "mx-auto mb-2 text-slate-350" />
            <p className="text-xs font-bold" > No active suggestions.</p>
                < p className = "text-[10px] mt-1" > Paste your resume and click Trigger Gemini diagnostics.</p>
                    </div>
                      )
}
</div>
...
<div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4" >
    <h4 className="text-xs font-bold uppercase tracking-wider text-slate-405 flex items-center gap-1.5" >
        <Target size={ 14 } className = "text-emerald-500" />
            Matching Alternative Roles
                </h4>
{
    aiAnalysis && aiAnalysis.role_recommendations && aiAnalysis.role_recommendations.length > 0 ? (
        <div className= "space-y-3" >
        {
            aiAnalysis.role_recommendations.map((rec, i) => (
                <div key= { i } className = "p-3 border border-slate-100 dark:border-slate-800 rounded-2xl bg-slate-50/20 dark:bg-slate-950/40" >
                <div className="flex justify-between items-center mb-1" >
            <h5 className="font-extrabold text-xs text-slate-850 dark:text-white" > { rec.title } </h5>
            < span className = "text-[10px] font-black text-emerald-500" > { rec.alignment } % Match </span>
            </div>
...
{/* 3. Mock Interview Arena Tab */ }
{
    activeTab === 'interviews' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" >
            {/* Left Sidebar containing the generated questions list */ }
                    ...
                    <Sparkles size={ 16 } className = "text-indigo-500" />
                        Mock Interview Arena
                            ...
                            < p className = "text-slate-400 text-[11px] leading-relaxed" >
                                Select an AI - tailored technical interview question matching your target track({ activeCareer.title }).
                    </p>

    {
        aiAnalysis && aiAnalysis.mock_interview_questions && aiAnalysis.mock_interview_questions.length > 0 ? (
            <div className= "space-y-2" >
            {
                aiAnalysis.mock_interview_questions.map((q, idx) => (
                    <div
                            key= { idx }
                            onClick = {() => {
                    setSelectedQuestion(q);
                            ...
                            className = {`p-3 rounded-xl border cursor-pointer text-xs transition-all flex items-center justify-between group ${selectedQuestion?.question === q.question
                ? 'border-indigo-500 bg-indigo-500/5'
                : 'border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-950/60'
            }`
    }
                          >
        <div>
        <p className="font-bold text-slate-750 dark:text-slate-350 line-clamp-2" > { q.question } </p>
            < span className = "text-[9px] text-indigo-500 bg-indigo-500/10 px-2 py-0.5 rounded-md mt-1.5 inline-block font-bold" >
                { q.topic }
                </span>
                </div>
                </div>
    ...
    <div className= "text-center py-8 text-slate-450 border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl" >
    <HelpCircle size={ 28 } className = "mx-auto mb-2 text-slate-350" />
        <p className="text-xs font-bold" > No questions loaded.</p>
            < p className = "text-[10px] mt-1" > Please paste your resume and run the AI diagnostics tool first.</p>
                </div>
                    )}
</div>

{/* Main Work Area: Answering and Review */ }
<div className="lg:col-span-2 space-y-6" >
{
    selectedQuestion?(
            ...
            < div className = "space-y-1" >
                <span className="text-[10px] font-black uppercase text-indigo-500 bg-indigo-500/10 px-2.5 py-0.5 rounded-md">
    Topic: { selectedQuestion.topic }
</span>
    < h3 className = "text-base font-extrabold text-slate-850 dark:text-white pt-2 leading-snug" >
...
<div className="space-y-2" >
    <label className="text-xs font-bold text-slate-500 block" > Your Answer Response </label>
        < textarea
placeholder = "Type your technical solution or explanation in detail..."
value = { userAnswer }
...
className = "w-full bg-slate-50/50 dark:bg-slate-950 border border-slate-200 dark:border-slate-850 rounded-2xl p-4 text-xs font-medium focus:outline-none focus:border-indigo-500 text-slate-750 dark:text-slate-300 placeholder-slate-400"
    ...
    < div className = "flex gap-4" >
        <button
                            onClick={ () => handleEvaluateAnswer(selectedQuestion.question, selectedQuestion.topic) }
disabled = { loadingEvaluation }
className = "px-6 py-2.5 bg-indigo-650 hover:bg-indigo-550 text-white rounded-xl text-xs font-bold cursor-pointer disabled:opacity-50"
    >
    { loadingEvaluation? "Grading...": "Submit Answer for AI Review" }
    </button>
    </div>

{/* Grading analysis display */ }
{
    evaluationResult && (
        <motion.div 
                            initial={ { opacity: 0, y: 10 } }
...
className = "p-5 border border-slate-100 dark:border-slate-850 rounded-2xl bg-slate-50/50 dark:bg-slate-950 space-y-4"
    >
    <div className="flex justify-between items-center pb-3 border-b border-slate-100 dark:border-slate-850" >
        <div>
        <span className="text-[10px] text-slate-400 block font-bold uppercase" > AI Evaluation Result </span>
            < h4 className = "font-extrabold text-sm text-slate-800 dark:text-white mt-1" > Verdict: { evaluationResult.verdict } </h4>
                </div>
                < div className = "text-right" >
                    <span className="text-2xl font-black text-indigo-600 dark:text-indigo-400" > { evaluationResult.score } / 100 </span>
                        < span className = "text-[9px] text-slate-400 block mt-0.5" > Response Score </span>
                            </div>
                            </div>

                            < div className = "grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-semibold leading-relaxed" >
                                <div>
                                <span className="text-[10px] text-slate-400 block font-bold uppercase mb-1" > Key Strengths </span>
                                    < p className = "text-emerald-500 bg-emerald-500/5 p-3 rounded-xl border border-emerald-500/10" >
                                        { evaluationResult.strengths }
                                        </p>
                                        </div>
                                        < div >
                                        <span className="text-[10px] text-slate-400 block font-bold uppercase mb-1" > Identified Gaps </span>
                                            < p className = "text-red-500 bg-red-500/5 p-3 rounded-xl border border-red-500/10" >
                                                ...
                                                < div >
                                                <span className="text-[10px] text-slate-400 block font-bold uppercase mb-1" > Suggested Model Solution </span>
                                                    < pre className = "p-4 bg-slate-950 text-indigo-300 font-mono text-[10px] overflow-x-auto rounded-xl whitespace-pre-wrap leading-relaxed" >
    ...
    <div className= "bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-12 text-center text-slate-450 h-full flex flex-col justify-center items-center" >
    <Sparkles size={ 36 } className = "text-indigo-500 mb-3 animate-pulse" />
        <h4 className="font-bold text-sm text-slate-700 dark:text-slate-350" > No Question Selected </h4>
            < p className = "text-xs text-slate-400 mt-1" > Select a question from the sidebar pipeline to begin the technical evaluation.</p>
                </div>
                    )}
</div>
    </div>
...
{/* 4. Deep Intelligence Tab */ }
{
    activeTab === 'intelligence' && (
        ...
        < p className = "text-slate-500 dark:text-slate-400 text-xs mt-2 leading-relaxed" >
            ...
            < div className = "grid grid-cols-1 lg:grid-cols-2 gap-8" >
                ...
                < div className = "space-y-4" >
                    <h4 className="text-xs font-bold uppercase tracking-wider text-slate-404 flex items-center gap-1.5" >
    ...
    {/* Tools Used */ }
    <div className="space-y-2.5" >
        <h4 className="text-xs font-bold uppercase tracking-wider text-slate-404" > Actual Industry Tools & Frameworks </h4>
            < div className = "flex flex-wrap gap-2" >
            {
                activeCareer.tools.map((tool, idx) => (
                    <span 
                              key= { idx }
                              className = "bg-slate-100 dark:bg-slate-800 border border-slate-200/50 dark:border-slate-800/80 text-slate-750 dark:text-slate-350 text-xs font-bold px-3 py-1 rounded-xl"
                    >
                    { tool }
                    </span>
                ))
    ...
    <div className="pt-6 border-t border-slate-100 dark:border-slate-850" >
        <h4 className="text-xs font-bold uppercase tracking-wider text-slate-404 mb-4" > Target Career Trajectory Progression </h4>
            < div className = "grid grid-cols-2 md:grid-cols-4 gap-4" >
            {
                ...
                < span className = "text-[10px] text-indigo-500 block font-bold" > Step { idx + 1 } </span>
                    < h5 className = "font-extrabold text-slate-850 dark:text-white text-xs mt-1 leading-tight" > { scope.level } </h5>
...
{/* 6. Salaries Tab */ }
{
    ...
    className = {`px-3 py-1.5 text-[11px] font-bold cursor-pointer ${salaryCurrency === 'INR'
...
className = {`px-3 py-1.5 text-[11px] font-bold cursor-pointer ${salaryCurrency === 'USD'
        ? 'bg-indigo-650 text-white'
        : 'bg-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-white'
    }`}
                      >
...
{/* 7. Company Intelligence Hub Tab */ }
{
    activeTab === 'companies' && (
        <div className="space-y-6" >
            {/* Search & Filter bar */ }
...
{
    aiAnalysis && aiAnalysis.company_recommendations && (
        <motion.div 
                      initial={ { opacity: 0 } }
    animate = {{ opacity: 1 }
}
className = "bg-gradient-to-r from-indigo-950 to-indigo-900 border border-indigo-500/25 p-6 rounded-3xl shadow-md text-white space-y-4"
    >
    <div className="flex items-center gap-2" >
        <Sparkles size={ 18 } className = "text-indigo-400" />
            <h4 className="text-sm font-bold uppercase tracking-wider text-indigo-300" > Gemini AI Suggested Recruiters for you </h4>
                </div>
                < div className = "grid grid-cols-1 md:grid-cols-2 gap-4" >
                {
                    aiAnalysis.company_recommendations.map((co, i) => (
                        <div key= { i } className = "p-4 bg-white/5 border border-white/10 rounded-2xl" >
                        <div className="flex justify-between items-center mb-1" >
                    <h5 className="font-extrabold text-sm text-indigo-300" > { co.name } </h5>
                    < span className = "text-[9px] bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded font-black uppercase" >
                    { co.suitability }
                    </span>
                    </div>
                    < p className = "text-xs text-indigo-100 font-medium" > Recommended: { co.role } </p>
                    </div>
                    ))
                }
                </div>
...
{/* Standard Companies Profiles Grid */ }
<div className="grid grid-cols-1 lg:grid-cols-2 gap-6" >
{
    ...
    < span className = {`text-[9px] font-black uppercase px-2 py-0.5 rounded-full ${c.difficulty === "Easy" ? 'bg-blue-100 text-blue-700 dark:bg-blue-950/40 dark:text-blue-400' :
...
{/* Hiring process */ }
<div>
    <span className="text-[10px] text-slate-404 font-bold uppercase tracking-wider block mb-2" > Hiring Pipeline Structure </span>
        < div className = "space-y-2" >
            {
...
<div>
    <span className="text-[10px] text-slate-450 font-bold uppercase tracking-wider block mb-2" > Common Skills Assessed </span>
        < div className = "flex flex-wrap gap-1.5" >
        {
            c.commonSkills.map((skill, idx) => (
                <span 
                                key= { idx }
                                className = "bg-slate-50 dark:bg-slate-950 border border-slate-150 dark:border-slate-850 text-slate-605 dark:text-slate-400 text-[10px] font-bold px-2.5 py-0.5 rounded-lg"
                >
                { skill }
                </span>
            ))
...
{/* Key Prep Areas */ }
<div>
    <span className="text-[10px] text-slate-450 font-bold uppercase tracking-wider block mb-2" > Interview Preparation Focus Areas </span>
        < ul className = "list-disc list-inside text-xs text-slate-555 dark:text-slate-400 space-y-1" >
        {
...
{/* 8. Projects & Resources Tab */ }
{
                    ...
                    <Layers size={ 16 } className = "text-indigo-500" />
                        Curated Project Recommendations({ activeCareer.title })
                          ...
                          className = "p-5 bg-slate-50 dark:bg-slate-950 border border-slate-150 dark:border-slate-850 rounded-2xl flex flex-col justify-between"
                                        ...
                                        < span className = "text-[10px] text-slate-450 font-semibold" > { proj.stack } </span>
                                        </div>

                                        < h4 className = "font-black text-slate-850 dark:text-white text-sm leading-tight" >
                                        { proj.title }
                                        </h4>
                                        < p className = "text-slate-500 dark:text-slate-400 text-xs mt-2 leading-relaxed" >
                                        { proj.desc }
    ...
    className = {`px-3 py-1 rounded-lg text-[10px] font-bold transition-all cursor-pointer ${resourceCategory === cat
...
}
{
    ...
    < h4 className = "font-extrabold text-slate-850 dark:text-white text-xs mt-2 leading-tight" >
        { res.title }
        </h4>
{
    res.provider && (
...
{/* 9. Placement Toolkit Tab */ }
{
                                    ...
                                    < div className = "p-5 border border-slate-150 dark:border-slate-850 rounded-2xl bg-slate-50/20 dark:bg-slate-950/40 space-y-3" >
                                        <h4 className="font-black text-slate-800 dark:text-white text-sm" >
                                            ...
                                            < div className = "space-y-2" >
                                            {
                                                ...
                                                < span className = "text-slate-550 dark:text-slate-400 leading-relaxed" > { sec.desc } </span>
                                                </div>
                                                ))
                                            }
                                                </div>
    ...
    {/* ATS Optimization */ }
    <div className="p-5 border border-slate-150 dark:border-slate-850 rounded-2xl bg-slate-50/20 dark:bg-slate-950/40 space-y-3" >
        <h4 className="font-black text-slate-850 dark:text-white text-sm" >
            { PLACEMENT_TOOLKIT.atsOptimization.title }
            </h4>
            < ul className = "list-disc list-inside text-xs text-slate-555 dark:text-slate-400 space-y-1.5 leading-relaxed" >
            {
    ...
    {/* Aptitude Prep */ }
    <div className="p-5 border border-slate-150 dark:border-slate-850 rounded-2xl bg-slate-50/20 dark:bg-slate-950/40 space-y-3" >
        <h4 className="font-black text-slate-855 dark:text-white text-sm" >
            ...
            < div className = "space-y-2" >
            {
                PLACEMENT_TOOLKIT.aptitudePrep.topics.map((topic, idx) => (
                    <div key= { idx } className = "text-xs" >
                    <span className="font-bold text-slate-700 dark:text-slate-205" > { topic.category }: </span>{" "}
                < span className = "text-slate-550 dark:text-slate-400 leading-relaxed" >
                { topic.topics.join(', ') }
                </span>
                </div>
                ))
...
{/* 10. Industry Trends Tab */ }
{
                    ...
                    <h3 className="font-extrabold text-slate-800 dark:text-white text-sm uppercase tracking-wider flex items-center gap-1.5" >
                        <TrendingUp size={ 16 } className = "text-indigo-500" />
                            Trending Industry Skills
                                ...
                                < p className = "text-slate-455 text-xs mt-1" > Skills displaying highest quarter - on - quarter demand growth.</p>
                                    </div>
    ...
    <div className="bg-white dark:bg-slate-905 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm space-y-4" >
        <div>
        <h3 className="font-extrabold text-slate-800 dark:text-white text-sm uppercase tracking-wider flex items-center gap-1.5" >
            <Cpu size={ 16 } className = "text-indigo-500" />
                Emerging Technologies
                    </h3>
                    < p className = "text-slate-455 text-xs mt-1" > Tech domains entering production cycles.</p>
                        </div>
        ...
        <h3 className="font-extrabold text-slate-800 dark:text-white text-sm uppercase tracking-wider flex items-center gap-1.5" >
            <BookOpen size={ 16 } className = "text-indigo-500" />
                Executive Industry Insights
                    ...
                    < p className = "text-slate-455 text-xs mt-1" > Expert opinions on company placement requirements.</p>
                        </div>

                        < div className = "space-y-3" >
                        {
                            INDUSTRY_TRENDS.industryInsights.map((insight, idx) => (
                                <div key= { idx } className = "p-4 bg-slate-50 dark:bg-slate-950 border border-slate-150 dark:border-slate-850 rounded-2xl text-xs leading-relaxed text-slate-755 dark:text-slate-350" >
                                { insight }
                                </div>
                            ))
                        }
...
{/* Internship Finder and Scraper Section (Unlocked) */ }
<div className="pt-6 border-t border-slate-200 dark:border-slate-800" >
    <div className="flex items-center gap-2 mb-4" >
        <Sparkles size={ 16 } className = "text-indigo-500" />
            <h3 className="font-extrabold text-slate-800 dark:text-white text-sm" > Gemini Internship Finder & Recommendation Pathways </h3>
                </div>

{
    aiAnalysis && aiAnalysis.internship_opportunities ? (
        <div className= "grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4" >
        {
            aiAnalysis.internship_opportunities.map((item, idx) => (
                <div 
                key= { idx } 
                className = "p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl relative overflow-hidden group hover:-translate-y-0.5 transition-all shadow-sm"
                >
                <div className="flex flex-col h-full justify-between gap-2" >
            <div>
            <div className="flex justify-between items-start" >
            <span className="text-[9px] text-indigo-500 font-bold uppercase" > { item.company } </span>
            < span className = "text-[9px] bg-indigo-500/10 text-indigo-500 px-2 py-0.5 rounded font-bold" > Internship </span>
            </div>
            < h4 className = "font-extrabold text-slate-850 dark:text-white text-xs mt-1.5 leading-tight" > { item.title } </h4>
            < p className = "text-[10px] text-slate-500 dark:text-slate-400 leading-relaxed mt-1.5" > { item.description } </p>
            </div>
            < a
                    href = { item.link }
                    target = "_blank"
                    rel = "noopener noreferrer"
                    className = "w-full py-1.5 bg-slate-100 dark:bg-slate-850 hover:bg-slate-200 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-750 text-slate-750 dark:text-slate-300 rounded-lg text-[9px] font-bold flex items-center justify-center gap-1 transition-colors"
                >
                Apply Now
            < ExternalLink size = { 8} />
            </a>
        ...
        {
            [
            { title: "NVIDIA ML Intern", desc: "PyTorch model deployment." },
            { title: "Supabase Backend Intern", desc: "PostgreSQL & Node APIs." },
            { title: "Google Engineering Intern", desc: "Data structures & scale." },
            { title: "Microsoft Software Intern", desc: "Cloud features & microservices." },
            { title: "TCS Systems Graduate", desc: "Enterprise systems stack." },
            { title: "Cognizant Developer Track", desc: "Full stack web integrations." }
            ].map((item, idx) => (
                <div 
                key= { idx } 
                className = "p-4 bg-slate-100/50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-900 rounded-2xl relative overflow-hidden group select-none opacity-60"
                >
                <div className="flex flex-col h-full justify-between gap-1.5" >
            <div>
            <span className="text-[9px] text-slate-400 block font-bold uppercase" > { item.title.split(' ')[0] } </span>
            < h4 className = "font-extrabold text-slate-650 dark:text-slate-450 text-[10px] mt-1 leading-tight" > { item.title } </h4>
            </div>

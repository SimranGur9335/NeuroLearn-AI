    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center min-h-[30vh] space-y-4">
          ...
          <span className="text-slate-400 text-xs animate-pulse">Loading coursework deliverables...</span>
        </div>
      ) : error ? (
        ...
        <div className="bg-slate-900 border border-slate-800 p-12 rounded-2xl text-center text-slate-500 text-sm font-semibold">
          ...
          {filteredAssignments.map((assign) => {
            ...
            return (
              <div 
                ...
                className="bg-slate-900/60 border border-slate-800 rounded-2xl p-6 hover:border-slate-750 transition-all duration-300 flex flex-col lg:flex-row lg:items-stretch justify-between gap-6"
              >
                {/* Left side details */}
                <div className="flex-1 space-y-4">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="text-xs text-slate-400 font-bold">{assign.subject_name}</span>
                    <span className="text-[10px] text-slate-500 font-semibold ml-auto sm:ml-0">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed whitespace-pre-wrap">{assign.description}</p>
                  </div>

                  {/* Supporting Material Card */}
                  {assign.attachment_url && (
                    <div className="bg-slate-950 border border-slate-800 p-3.5 rounded-xl flex items-center justify-between max-w-lg">
                      <div className="flex items-center gap-3">
                        <div className="p-2 bg-slate-900 rounded-lg text-indigo-400">
                          <FileText size={18} />
                        </div>
                        <div>
                          <p className="text-xs font-bold text-slate-200 line-clamp-1">{assign.attachment_name || 'Resources Material'}</p>
                      ...
                      <a 
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="bg-slate-900 hover:bg-slate-850 p-2 rounded-lg text-slate-400 hover:text-white border border-slate-800 transition-colors cursor-pointer"
                  ...
                  {/* Instructions Box */}
                  {assign.instructions && (
                    <div className="border border-slate-800 bg-slate-950/40 p-4 rounded-xl space-y-1.5">
                      ...
                      <p className="text-xs text-slate-450 leading-relaxed font-mono whitespace-pre-wrap">
                        {assign.instructions}
                      </p>
                    </div>
                  )}
                ...
                <div className="shrink-0 w-full lg:w-80 pt-6 lg:pt-0 border-t lg:border-t-0 lg:border-l border-slate-800 lg:pl-6 flex flex-col justify-center gap-3">
                  {['Submitted', 'Late', 'Graded'].includes(assign.status) ? (
                    <div className="space-y-3.5">
                      <span className="text-[10px] text-slate-500 block uppercase font-black tracking-wider">Your Deliverables</span>
                        ...
                        <div className="bg-slate-850 p-3 rounded-xl border border-slate-800 flex items-center justify-between">
                          <div className="flex items-center gap-2 min-w-0">
                            ...
                            <div className="truncate">
                              <span className="text-[9px] text-slate-500 block uppercase font-bold">Uploaded File</span>
                          ...
                          <a 
                            ...
                            target="_blank" 
                            rel="noreferrer"
                            className="bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white p-2 rounded-lg border border-slate-700 transition-colors"
                          >
                      ...
                      {assign.external_url && (
                        <div className="bg-slate-850 p-3 rounded-xl border border-slate-800 flex items-center justify-between">
                          <div className="flex items-center gap-2 min-w-0">
                            <Link className="text-indigo-400" size={16} />
                            <div className="truncate">
                              <span className="text-[9px] text-slate-500 block uppercase font-bold">External URL</span>
                              <span className="text-xs text-slate-300 font-bold truncate block">
                                {assign.external_url}
                              </span>
                            </div>
                      ...
                      {assign.assignment_status !== 'Closed' && (
                        <button
                          ...
                          className="w-full bg-slate-800 hover:bg-slate-700 text-white font-extrabold py-2.5 px-4 rounded-xl text-xs flex items-center justify-center gap-2 transition-all cursor-pointer"
                        >
                          Re-Submit Work
                        </button>
                      )}
                    ...
                    <div className="space-y-3.5">
                      <span className="text-[10px] text-slate-400 uppercase font-black block">Upload Deliverables</span>
                      
                      {/* File uploader */}
                      <label className="w-full flex flex-col items-center justify-center bg-slate-950 hover:bg-slate-900 border-2 border-dashed border-slate-800 hover:border-slate-600 rounded-xl p-4 cursor-pointer transition-all">
                        <UploadCloud className="text-slate-500 mb-1" size={22} />
                        ...
                        <span className="text-[9px] text-slate-500 mt-0.5">PDF, DOCX, ZIP etc. (Max 15MB)</span>
                        <input 
                          type="file" 
                          className="hidden" 
                      ...
                      <div>
                        ...
                        <div className="relative">
                          <Link className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500" size={13} />
                          <input
                            type="url"
                            ...
                            className="w-full bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-3 py-2 text-xs text-slate-300 focus:outline-none focus:border-indigo-500 transition-colors"
                          />
                        </div>
                      </div>

    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center min-h-[30vh] space-y-4">
          ...
          <span className="text-slate-400 text-xs animate-pulse">Loading subject enrollments...</span>
        </div>
      ) : error ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map((course, idx) => (
            <div 
              key={idx}
              className={`bg-slate-900/60 border border-slate-800 rounded-2xl p-6 hover:border-slate-700 transition-all duration-300 relative group overflow-hidden`}
            >
              <div className="absolute right-0 top-0 -mr-4 -mt-4 w-20 h-20 bg-white/[0.01] rounded-full group-hover:bg-white/[0.03] transition-colors" />
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <h3 className="font-extrabold text-base text-white leading-snug group-hover:text-slate-200 transition-colors">
                    {course.subject_name}
                  </h3>
                ...
                <div className="pt-4 border-t border-slate-800/80 flex items-center gap-2.5">
                  <div className={`p-2 rounded-lg bg-slate-850 ${theme.text} shrink-0`}>
                    <User size={14} />
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-500 block uppercase font-bold tracking-wider">Assigned Faculty</span>
                    <span className="text-xs font-bold text-slate-300">{course.faculty_name}</span>
                  </div>
                </div>
              </div>
            </div>

    ...
    <div className="space-y-8">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center min-h-[30vh] space-y-4">
          ...
          <span className="text-slate-400 text-xs animate-pulse">Retrieving attendance logs...</span>
        </div>
      ) : error ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 flex flex-col items-center justify-between text-center space-y-6">
            <h3 className="font-extrabold text-sm text-slate-400 uppercase tracking-wider">Overall Standing</h3>
            ...
            <div className="relative w-40 h-40 flex items-center justify-center">
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-3xl font-black text-white">{data.overall_percentage}%</span>
                <span className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">Total Presence</span>
              </div>
            </div>

            <div className="w-full bg-slate-850 p-4 rounded-2xl border border-slate-800/80 text-left flex items-start gap-3">
              <AlertCircle className={data.overall_percentage >= 75 ? 'text-emerald-400 shrink-0' : 'text-amber-400 shrink-0'} size={20} />
              <div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 lg:col-span-2 flex flex-col justify-between space-y-4">
            <div>
                ...
                <div key={idx} className="space-y-2">
                  <div className="flex justify-between text-xs">
                    <div className="space-x-2">
                      <span className="font-extrabold text-white">{sub.subject_name}</span>
                      <span className="text-[10px] text-slate-500 font-bold uppercase">{sub.subject_code}</span>
                    </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-6 overflow-hidden flex flex-col justify-between">
            <h3 className="font-extrabold text-sm text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
            ...
            <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1">
              {data.history.length === 0 ? (
                ...
                data.history.slice(0, 10).map((log, idx) => (
                  <div key={idx} className="flex items-center justify-between gap-4 p-2 rounded-xl hover:bg-slate-850 transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <span className="text-xs font-extrabold text-white block">{log.subject_name}</span>

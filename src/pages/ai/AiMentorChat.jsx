...
During backpropagation, the gradient of the loss function $L$ with respect to weight $w_1$ in the first layer is computed using the Chain Rule:
$$ \\frac{\\partial L}{\\partial w_1} = \\frac{\\partial L}{\\partial a_d} \\times \\frac{\\partial a_d}{\\partial a_{d-1}} \\times \\dots \\times \\frac{\\partial a_2}{\\partial a_1} \\times \\frac{\\partial a_1}{\\partial w_1} $$

If the activation functions (like Sigmoid or Tanh) have derivatives strictly less than 1 ($f'(x) \\le 0.25$ for Sigmoid), multiplying many of these terms together causes the product to approach 0. Consequently, the weights of early layers update extremely slowly, halting learning.
...
4. **Input Validation**: Use schemas (e.g., Zod, Joi) to validate and sanitize incoming payloads.`,
...
1. **Ingress Layer**: Nginx Ingress Controller routing HTTP telemetry packets to the K8s cluster.
...
const AiMentorChat = () => {
  const [messages, setMessages] = useState([
    { role: 'assistant', text: "Hello! I am your AI Academic Mentor. Ask me any engineering concepts, request Capstone project architectures, or get help debugging code configurations. Click a suggested query below to begin.", date: new Date().toLocaleTimeString() }
  ]);
  ...
  const handleSend = (textToSend) => {
    ...
    setTimeout(() => {
      let responseObj = {
        response: "That's an interesting technical question! I can help you model that concept, draft an architecture, or review configurations. Please click one of the suggested prompts or refine the wording for a targeted answer.",
        code: null
      };

      // Match scripted answers
      if (AI_RESPONSES[text]) {
        responseObj = AI_RESPONSES[text];
      }
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 h-[calc(100vh-120px)]">
      ...
      <div className="hidden lg:flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-5 shadow-sm justify-between">
        <div>
          ...
          <div className="space-y-2">
            {history.map((hist, idx) => (
              <div 
                key={idx}
                className="p-3 rounded-xl border border-slate-100 dark:border-slate-850 hover:bg-slate-50 dark:hover:bg-slate-950/60 cursor-pointer text-xs transition-colors"
              >
        ...
        <div className="bg-slate-50 dark:bg-slate-950/80 p-3 rounded-xl border border-slate-100 dark:border-slate-850 flex items-center gap-2 text-[10px] text-slate-500">
      ...
      <div className="lg:col-span-3 flex flex-col bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-md overflow-hidden h-full">
        {/* Chat Header */}
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-950/40 border-b border-slate-100 dark:border-slate-850 flex justify-between items-center">
          ...
          <AnimatePresence initial={false}>
                ...
                <div key={index} className={`flex gap-3 max-w-[85%] ${isAi ? 'self-start' : 'self-end flex-row-reverse ml-auto'}`}>
                  ...
                  <div className="space-y-2">
                    <div className={`p-4 rounded-3xl text-xs leading-relaxed ${
                      isAi 
                        ? 'bg-slate-50 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-850 text-slate-700 dark:text-slate-350 rounded-tl-sm' 
                      ...
                      {msg.code && (
                        <div className="mt-4 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm">
                          <div className="bg-slate-100 dark:bg-slate-950 px-4 py-2 border-b border-slate-200 dark:border-slate-850 flex justify-between items-center text-[9px] font-mono text-slate-500">
            ...
            {isTyping && (
              <div className="flex gap-3 max-w-[80%] self-start">
                ...
                <div className="p-4 rounded-3xl bg-slate-50 dark:bg-slate-950/40 border border-slate-100 dark:border-slate-850 text-slate-400 text-xs italic flex items-center gap-1.5 rounded-tl-sm">
        ...
        {/* Suggestion Prompts (if chat has few messages) */}
        {messages.length === 1 && (
          <div className="px-6 py-3 border-t border-slate-100 dark:border-slate-850 space-y-2 bg-slate-50/50 dark:bg-slate-950/20">
            <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-wider mb-1 flex items-center gap-1">
              <HelpCircle size={12} />
            ...
            <div className="flex flex-wrap gap-2">
              {Object.keys(AI_RESPONSES).map((prompt, idx) => (
                <button
                  ...
                  className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-indigo-500 dark:hover:border-indigo-500 text-slate-700 dark:text-slate-350 text-[11px] font-bold px-3 py-2 rounded-xl transition-all cursor-pointer text-left"
                >
                  {prompt}
                </button>
              ))}
        ...
        {/* Input Bar */}
        <div className="p-4 border-t border-slate-100 dark:border-slate-850 bg-slate-50 dark:bg-slate-950/30 flex gap-2 items-center">
          <input 
            type="text" 
            ...
            className="flex-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-xs text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all"
          />
          <button 
            onClick={() => handleSend()}
            className="bg-indigo-600 hover:bg-indigo-500 text-white p-3 rounded-xl shadow-md transition-colors cursor-pointer"
          >
            <Send size={16} />
          </button>
        </div>

...
const CourseManagement = () => {
  const { coursesList, addCourse, updateCourse, deleteCourse } = useStudent();
  ...
  const handleSaveAdd = (e) => {
    ...
    const newCourse = {
      id: `c-${coursesList.length + 1}`,
      code: formCode,
      title: formTitle,
      department: formDept,
      category: formCategory,
      duration: formDuration,
      enrollment: Math.floor(Math.random() * 80) + 40
    };
        ...
        {filteredCourses.map((course) => (
          ...
          >
            <div>
              ...
              <h3 className="font-extrabold text-slate-800 dark:text-white text-sm md:text-base leading-snug">{course.title}</h3>
            ...
            <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-850 flex justify-between items-center text-xs">
              <div className="flex items-center gap-1.5 text-slate-400">
                <Users size={14} />
                <span className="font-bold">{course.enrollment} Enrolled</span>
              </div>
